...
function GalleryCanvas({ items }) {
    ...
    const filtered = search.trim()
        ? items.filter(i => i.tags.some(t => t.includes(search.toLowerCase())) || i.label.toLowerCase().includes(search.toLowerCase()))
        : items;
    ...
    const layout = filtered.map((item, i) => {
        ...
        for (let c = 1; c < COLS; c++) {
            if (colHeights[c] < minH) { minCol = c; minH = colHeights[c]; }
        }
    ...
    const onMouseMove = (e) => {
        ...
        velRef.current = { x: e.clientX - lastPos.current.x, y: e.clientY - lastPos.current.y };
        lastPos.current = { x: e.clientX, y: e.clientY };
    ...
    const onTouchMove = (e) => {
        ...
        velRef.current = { x: clientX - lastPos.current.x, y: clientY - lastPos.current.y };
    ...
    return (
        <div>
            ...
            <div className="flex gap-2 justify-center mb-8 flex-wrap">
                {["concert", "led", "lighting", "rigging", "stage", "laser", "festival", "ceremony"].map(tag =>
                    <button key={tag}
                        ...
                        className={clsx(
                            ...
                            search === tag ? "border-accent bg-accent text-navy font-bold shadow-[0_0_15px_rgba(212,224,237,0.3)]" : "border-[#d0daf0] bg-transparent text-[#4a5a7a] hover:border-accent hover:text-accent"
                        )}
                    >{tag}</button>
                )}
            </div>
            ...
            <div
                ...
                className={clsx(
                    "w-full h-[600px] overflow-hidden relative rounded-xl bg-white border border-[#e8eef8] select-none touch-none",
            ...
            >
                <div className="absolute" style={{
                    ...
                    {layout.map((item) => (
                        ...
                        >
                            <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${item.color} 0%, rgba(212,224,237,0.1) 100%)` }} />
                            ...
                            <div className="absolute bottom-4 left-4 right-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {item.tags.map(t => (
                                        <span key={t} className="px-2 py-0.5 rounded-[4px] bg-[#d4e0ed]/10 text-[#d4e0ed] text-[10px] font-mono uppercase tracking-wider">{t}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
