...
export default function CustomCursor({ color = "#d4e0ed" }) {
    ...
    useEffect(() => {
        ...
        const hover = (e) => {
            ...
            if (t) { setLabel(t.dataset.cursor); setExpanded(true); }
    ...
    return (
        <>
            <div ref={cursorRef} className="fixed top-0 left-0 w-3 h-3 rounded-full pointer-events-none mix-blend-multiply z-[9999]" style={{
                background: color, transition: "transform 0.05s linear",
            }} />
            <div ref={followerRef} className="fixed top-0 left-0 rounded-full border-[1.5px] pointer-events-none flex items-center justify-center font-semibold tracking-wider z-[9998]" style={{
                ...
                {expanded && label && <span className="whitespace-nowrap text-[9px]">{label}</span>}
            </div>
        </>
    );
}
